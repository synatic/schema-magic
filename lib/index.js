...
class SchemaMagic {
    ...
    static flattenSchema(schema, options) {
        ...
        options = options || {};
        const arrayItemIndicator = options.arrayItemIndicator ?? 'n';
        const sep = options.format === 'path' ? '/' : '.';
        const includeField = options.includeField ?? false;
        ...
        const traverseSchema = (curSchema, curPath, isArray, isRequired) => {
            ...
            if (curSchema.type === 'array') {
                if (curSchema.items) {
                    traverseSchema(curSchema.items, curPath.concat([arrayItemIndicator]), true, isRequired);
                } else {
                    const retSchema = {
                        path: $json.compilePath(curPath.concat([arrayItemIndicator]), sep),
                        isArray: true,
                        type: curSchema.type,
                        required: !!isRequired,
                    };
                    if (includeField) {
                        retSchema.field = curPath.at(-1);
                    }
                    if (includePaths) {
                        retSchema.paths = curPath;
                    }
    ...
    static generateSchemaFromJSON(obj) {
        const generateSchemaPart = (partObj) => {
            if (partObj === null) {
                ...
                const mergedItems = SchemaMagic.mergeSchemas(
                    partObj.map((i) => {
